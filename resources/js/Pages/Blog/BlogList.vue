...
const submitdata = () => {
    ...
    const routeName = isUpdate ? 'category.update' : 'category.store';
...
const deletedata = (dataId) => {
    ElMessageBox.confirm(
        ...
        dataForm.delete(route('category.destroy', dataId), {
            onSuccess: (page) => {
                data.value = page.props.data;
                ElMessage.success({
        ...
        <div class="min-h-screen w-full bg-gray-50 px-6 py-8">
            <div class="max-w-7xl mx-auto">
                ...
                <input v-model="searchQuery"
                    ...
                    <el-button class="ml-3" tag="a" :href="route('blogexamform')" type="success"> Add </el-button>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="(item, index) in filtereddata" :key="index" class="hover:bg-blue-50 transition-colors duration-150">
                                    ...
                                    <td class="px-8 py-4 whitespace-nowrap text-sm text-gray-700">
                                        {{ item.questions[0]?.academic_class.name }}
                                    </td>
                                    <td class="px-8 py-4 whitespace-nowrap text-sm text-gray-700">
                                        {{ item.questions[0]?.subject.name }}
                                    </td>
                                    <td class="px-8 py-4 whitespace-nowrap text-sm text-gray-700">
                                        {{ item.questions[0]?.lavel.name }}
                                    </td>
                                    <td class="px-8 py-4 whitespace-nowrap text-sm text-gray-700">
                                        {{ item.questions.length}}টি
                                    </td>
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-6 pt-6 pb-4">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">
                            {{ dataForm.id ? 'Edit Category' : 'Add New Category' }}
                        </h3>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="e.g. question category"
                                >
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-end space-x-3">
                        ...
                        >
                            {{ dataForm.id ? 'Update Category' : 'Add Category' }}
